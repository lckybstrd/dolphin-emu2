...
void JitArm64::crXXX(UGeckoInstruction inst)
{
  ...
  gpr.BindCRToRegister(field, true);
  ARM64Reg field_reg = gpr.CR(field);

  if (bit != PowerPC::CR_GT_BIT)
    FixGTBeforeSettingCRFieldBit(field_reg);

  switch (bit)
  {
  case PowerPC::CR_SO_BIT:  // set bit 59 to input
    BFI(field_reg, XA, PowerPC::CR_EMU_SO_BIT, 1);
    break;

  case PowerPC::CR_EQ_BIT:  // clear low 32 bits, set bit 0 to !input
    AND(field_reg, field_reg, LogicalImm(0xFFFF'FFFF'0000'0000, GPRSize::B64));
    EOR(XA, XA, LogicalImm(1, GPRSize::B64));
    ORR(field_reg, field_reg, XA);
    break;

  case PowerPC::CR_GT_BIT:  // set bit 63 to !input
    EOR(XA, XA, LogicalImm(1, GPRSize::B64));
    BFI(field_reg, XA, 63, 1);
    break;

  case PowerPC::CR_LT_BIT:  // set bit 62 to input
    BFI(field_reg, XA, PowerPC::CR_EMU_LT_BIT, 1);
    break;
  }

  ORR(field_reg, field_reg, LogicalImm(1ULL << 32, GPRSize::B64));
}
